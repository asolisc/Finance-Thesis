...
Before we continue, we must extract the so called *time series index*. In this case, the variable by which the time series is indexed is the `tidy_date` variable.

```{r get-index}
# We'll use the tk_index() function from the timetk package.
index_date <- IPC_tbl %>% 
  timetk::tk_index(timetk_idx = tidy_date)
...
We can then extract several time-related variables from this time series index.
