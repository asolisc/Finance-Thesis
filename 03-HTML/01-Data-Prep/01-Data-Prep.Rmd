...
output: 
  distill::distill_article:
    toc: true
    toc_depth: 2
    css: "D:/11-Estancia/03-Finance-Thesis/05-Resources/styles.css"
editor_options: 
...
First we drop the `ticker`, `type`, `open`, `high`, and `low` columns because we think they are of no use for the analysis.

Also, we create a `tidy_date` variable where we store the date according to the _ISO 8601_ standard that states that dates should be expressed in the `YYYY-MM-DD` format. In consequence, the `raw_date` column is dropped and we keep the newly created `tidy_date` variable instead.

We store the modified data into the `IPC_tbl` (IPC tibble) object.

```{r}
IPC_tbl <- IPC_ip %>% as_tibble() %>%
  select(-c(ticker, type, open, high, low)) %>% 
