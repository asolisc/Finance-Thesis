...
3.  The `ticker`, `type`, `open`, `high`, and `low` columns are removed because (we think )they are of no use for the analysis.
...
IPC_tbl <- IPC_ip %>% 
  ...
  # Remove some columns.
  select(-c(raw_date, raw_time, ticker, type, open, high, low)) %>% 
...
Before we continue, we must create the so called **time series index**. To achieve this, a `dttm` (*datetime*) variable is created, which results from combining the `tidy_date` variable and `tidy_time` variable. This combined variable gets the name of `tidy_dttm`. In this case, the variable by which the time series will be indexed is the `tidy_dttm` variable. Below we print a small sample (10 rows) of this variable:

```{r}
IPC_tbl <- IPC_tbl %>% 
  
  # Create time series index: tidy_dttm.
...
-   `second`: The *second* component of the time `index` (from 0 - 59).
...
-   `week`: The week number of the year (Monday is considered the first day).
...
```{r}
ts_signature <- IPC_tbl %>% 
  select(tidy_dttm) %>% 
  
  # Rename tidy_dttm variable as "index"
  rename(index = tidy_dttm) %>% 
  ...
  mutate(
    ...
    # mweek       =
    # week        = lubridate::week(),
    date_change = if_else(index_date - lag(index_date) == 0, FALSE, TRUE),
    mday7       = day %% 7
  )
```
...
`tidy_minute`: a `dbl` that stores the minute of the reported price (from 0 through 59).

`tidy_time`: an `hms` (hour-minute-second) object that stores the time of the reported price.
