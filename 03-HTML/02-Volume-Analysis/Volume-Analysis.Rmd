...
```{r}
scale_ghibli_d <- function() {
  ghibli::scale_fill_ghibli_d("YesterdayMedium", direction = -1)
}

scale_ghibli_c <- function() {
  ghibli::scale_fill_ghibli_c("YesterdayMedium", direction = -1)
}

colour_ghibli_d <- function() {
  ghibli::scale_color_ghibli_d("YesterdayMedium", direction = -1)
}

colour_ghibli_c <- function() {
  ghibli::scale_color_ghibli_c("YesterdayMedium", direction = -1)
}

options

options(ggplot2.discrete.fill = scale_ghibli_d(),
        ggplot2.continuous.fill = scale_ghibli_c(),
        ggplot2.discrete.colour = colour_ghibli_d(),
        ggplot2.continuous.colour = colour_ghibli_c()
        )
```


```{r}
# --- setup the error search engine ---
...
volume_data <- read_parquet(file = "01_data/volume.parquet")
```
...
p1 <- volume_data %>% 
  ...
  ggplot(aes(tidy_time, volume)) +
   geom_col(alpha = 0.9, fill = tidyquant::palette_light()[6]) +
  scale_x_time(
...
p3 <- volume_data %>% 
  ...
  geom_col(alpha = 0.9, fill = tidyquant::palette_light()[1]) +
  # geom_smooth(
  #   formula = y ~ x, 
  #   method = 'loess'
  #   ) +
...
volume_clean %>%
  ...
  geom_histogram(fill = tidyquant::palette_light()[6],
                 colour = tidyquant::palette_light()[1],
                 bins = 50) +
  scale_y_continuous(
    labels = scales::number_format(big.mark = ",")
  ) +
...
volume_subgrid %>%
  ...
  geom_col(fill = "#26432FFF") +
  
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  
  scale_y_continuous(labels = scales::number_format(big.mark = ",")) +
  
  labs(x = "Time Grid (half-hour)",
       y = "Average Volume per Grid")
```
