...
For this section, we'll transform our dataset into a `timetibble` object, so that time filtering and wrangling of the data is easier. We'll call the new dataset `IPC_fr` (IPC tibble time).
...
```{r}
get_RV_sparse <- function(.grid_vec, 
                          .ret_tbl = IPC_fr, 
                          .index_by = index_date) {
  ...
  new_ret_tbl <- .ret_tbl %>% 
    ...
    slice(seq(1, nrow(.ret_tbl), by = .grid_vec))
  ...
  new_RV_tbl <- new_ret_tbl %>% 
    ...
    group_by({{ .index_by }}) %>% 
    summarise(
      "RVs{{ .grid_vec }}" := sum(log_ret_sq)
      # "RVs{{ .grid_vec }}_an" := 252 * sqrt("RVs{{ .grid_vec }}")
      )
...
sparse_grid <- seq(1, 5, 1)


# Create a tibble with all the nested data
all_RV_sparse <- tibble(
  my_result = x %>% map(.f = ~ get_RV_sparse(.grid_vec = .))
)

# Expand the tibble
# all_RV_sparse %>% flatten()
...
```{r}
all_RV_sparse <- RVs %>% 
  left_join(y = RV_s5, by = "index_date") %>% 
  left_join(y = RV_s10, by = "index_date") %>% 
  left_join(y = RV_s15, by = "index_date") %>% 
  left_join(y = RV_s30, by = "index_date")
