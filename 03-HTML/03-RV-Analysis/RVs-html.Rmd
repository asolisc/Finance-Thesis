...
rv_plot <- RVs %>% 
  ...
  labs(
    ...
    subtitle = "As measured by the annualized daily-realized-volatility",
    x = NULL,
    y = "Annualized RV"
    ) +
...
get_RV_sparse <- function(.grid_vec, 
  ...
  new_RV_tbl <- new_ret_tbl %>% 
    ...
    summarise(
      ...
      RV_sparse_an = 252 * sqrt(RV_sparse),
      .groups = "drop"
      # "RVs{{ .grid_vec }}" := sum(log_ret_sq),
      # "RVs{{ .grid_vec }}_an" := 252 * sqrt(eval(as.name(paste0("RVs", {{.grid_vec}}))))
      )
...
nested_RV_sparse <- tibble(
  my_result = sparse_grid %>% map(.f = ~ get_RV_sparse(.grid_vec = .)),
  s_grid = sparse_grid
)
```
